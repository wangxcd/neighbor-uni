...
* @Date: 2025-07-16 21:48:52
* @LastEditors: wangxc
* @LastEditTime: 2025-07-16 21:48:52
-->
<template>
  <view class="user-coupon-buy-box">
      ...
      <view class="header sec-box">
        <view class="headerCon">
          <view class="account acea-row row-top">
            <view class="assets">
              <view>消费券余额(元)</view>
              <view class="money">{{ userAct.couponLast ? userAct.couponLast.toFixed(2) : '0.00' }}</view>
            </view>
            <view class="assets">
              <view>消费券数量(张)</view>
              <view class="money">{{ userAct.couponCount ? userAct.couponCount : '0' }}</view>
            </view>
          </view>
        </view>
      </view>
        ...
        <view class="payment">
          ...
          <view class="pic-list">
            <view class="pic-box" :class="activePick === index ? 'box-active' : ''" v-for="(item, index) in prodDefList"
              ...
              <view class="pic-number-pic"> {{ item.price * item.prodNum }}<span class="pic-number"> 元券</span> </view>
              <view class="gift-list" v-if="item.isGift">
                <view class="tip">送</view>
                <view class="bundle">
                  <view class="gift" v-if="item.giftCash && item.giftCash > 0">
                    {{ item.giftCash }}元现金
                  </view>
                  <view class="gift" v-if="item.giftCouponAmount && item.giftCouponAmount > 0">
                    {{ item.giftCouponAmount }}元消费券
                  </view>
                  <view class="gift" v-if="item.bundleIntegral && item.bundleIntegral > 0">{{ item.bundleIntegral }}积分
                  </view>
                </view>
              </view>
            </view>
          ...
          <view class="tips-box">
            ...
            <view class="tips-samll" v-for="item in couponBuyNotes" :key="item">
              {{ item }}
            </view>
          </view>
          <button class="but" formType="submit">立即购买</button>
        </view>
      </form>
    </view>
          ...
          if (res.code === 0) {
            ...
            if (prodDefineList) {
              _that.prodDefList = prodDefineList.map(item => {
                item.isGift = false
                if ((item.giftCash && item.giftCash > 0) || (item.giftIntegral && item.giftIntegral > 0)) {
                  item.isGift = true
                }

                if (isNotEmpty(item.giftCoupon)) {
                  const coupons = item.giftCoupon.split('+')
                  if (coupons.length > 0) {
                    let couponAmount = 0
                    for (let i = 0; i < coupons.length; i++) {
                      const val = parseInt(coupons[i].trim())
                      if (val != NaN) {
                        couponAmount += val
                      }
                    }
                    ...
                    if (couponAmount > 0) {
                      item.isGift = true
                      item.giftCouponAmount = couponAmount
                    }
                  }
                }
                return item;
            ...
            if (couponBuyNotes) {
              _that.couponBuyNotes = couponBuyNotes
            }
          }
        })
      },
      ...
      submitSub(e) {
        ...
        buyProdFormPreOrder(req).then((res) => {
          ...
          if (res.code === 0) {
            if (res.data) {
              const _res = res.data
              _that.goPay(_res.orderId, _res.feeAmount, 0)
            }
          }
        })
      },
